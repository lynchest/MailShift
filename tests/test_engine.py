...
def test_pro_analyze_sil_decision():
    ...
    with patch("mailshift.core.analyzers.pro._get_session") as mock_session_fn:
        ...
        mock_session.post.return_value = _mock_ollama_response('{"decision":"SIL","reason":"pazarlama"}')
        result = pro_analyze(meta, _make_ollama_cfg())
    assert result.decision == "SIL"
    assert "SIL" in result.reason
...
def test_pro_analyze_tut_decision():
    ...
    with patch("mailshift.core.analyzers.pro._get_session") as mock_session_fn:
        mock_session = MagicMock()
        mock_session_fn.return_value = mock_session
        mock_session.post.return_value = _mock_ollama_response('{"decision":"TUT","reason":"toplantı notu"}')
        result = pro_analyze(meta, _make_ollama_cfg())
    assert result.decision == "TUT"
    assert "TUT" in result.reason
...
def test_pro_analyze_truncates_body_to_max_chars():
    ...
    def capture_post(url, json, timeout):
        ...
        return _mock_ollama_response('{"decision":"TUT","reason":"ok"}')
    ...
    # Header + body + prompt yapısı gereği 500 char body, prompt ile 900 civarı eder
    assert len(user_msg) <= 1200
...
def test_engine_analyze_pro_mode_llm_overrides_heuristic_tut():
    ...
    with patch("mailshift.core.analyzers.pro._get_session") as mock_session_fn:
        mock_session = MagicMock()
        mock_session_fn.return_value = mock_session
        mock_session.post.return_value = _mock_ollama_response('{"decision":"TUT","reason":"ok"}')
        results, stats = engine.analyze(mails)
